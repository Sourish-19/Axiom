  ...
  return (
    <section className="md:py-16 md:px-4">
      <div className="container mx-auto">
        <div className="flex flex-row justify-around items-center text-center gap-2 md:gap-10">
          {stats.map((stat, index) => (
            <div key={index} className="relative w-full md:w-auto">
              ...
              <p className="text-2xl md:text-5xl font-bold text-white">{stat.value}</p>
              <p className="mt-2 text-xs md:text-lg text-gray-400">{stat.label}</p>
            </div>
          ))}
        </div>
