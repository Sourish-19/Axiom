...
const Header: React.FC<HeaderProps> = ({ onContactClick }) => {
  ...
  return (
    ...
    >
      ...
      {showNavlinks && (
        <div
          className="lg:hidden absolute z-1 bg-black border-b border-white/30 backdrop-blur left-0 top-[80px] right-0 flex flex-col items-center pb-4 px-4"
        >
          {navLinks.map((link, idx) => {
            ...
            return (
              <a
                ...
                href={link.href}
                onClick={handleClick}
                className={`w-full text-center relative text-base text-gray-300 hover:text-white transition-colors py-2 ${idx < navLinks.length-1 ? "border-b border-white" : ""}`}
              >
                {link.name}
              </a>
            );
