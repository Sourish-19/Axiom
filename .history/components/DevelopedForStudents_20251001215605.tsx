...
const Card: React.FC<CardProps> = ({ title, content, className = '', hasGradient = false }) => {
  return (
    <div 
      className={`bg-[#181818] border border-gray-800/50 p-6 rounded-3xl group relative overflow-hidden h-full flex flex-col justify-between ${className}`}
    >
      {hasGradient && (
        <div aria-hidden="true" className="absolute top-1/3 left-1/3 size-full bg-[radial-gradient(circle,_#602614_0%,_transparent_100%)] blur-2xl"></div>
      )}
      
      <div className="absoluteflex justify-between items-start">
