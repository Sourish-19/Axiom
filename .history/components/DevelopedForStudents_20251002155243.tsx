...
const Card: React.FC<CardProps> = ({ title, content, className = '', hasGradient = false }) => {
  return (
    <div
      className={`bg-[#181818] h-[220px] border rounded-3xl group relative overflow-hidden ${className}`}
    >
      {hasGradient && (
      ...
      <div className='absolute z-2 top-0 bottom-0 left-0 right-0 p-6 flex flex-col justify-between'>
        ...
        <div>
          <h3 className="text-[32px] tracking-[-12px] font-thin">{title}</h3>
        </div>
      </div>
    </div>
  );
    ...
    <section id="about" className="py-20 px-4">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
          <Card
            className="md:col-span-2 border-gray-800/50"
            title="Mission"
          ...
          />
          <Card
            className="md:col-span-3 border-[#49271d]"
            title="Vision"
