...
const Header: React.FC<HeaderProps> = ({ onContactClick }) => {
  ...
  return (
    ...
    >
      ...
      {showNavlinks && (
        <div
          className="lg:hidden absolute z-1 bg-black border-b border-white/30 backdrop-blur left-0 top-[80px] right-0 flex flex-col gap-4 items-center pb-4"
        >
          {navLinks.map((link) => {
            const id = link.href.substring(1);
            const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
            ...
            return (
              <a
                ...
                href={link.href}
                onClick={handleClick}
                className={`relative text-base text-gray-300 hover:text-white transition-colors py-2 bg-white`}
              >
                {link.name}
              </a>
            );
