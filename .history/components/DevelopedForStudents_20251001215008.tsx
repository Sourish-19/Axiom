...
const Card: React.FC<CardProps> = ({ title, content, className = '', hasGradient = false }) => {
  return (
    <div 
      className={`bg-[#181818] border border-gray-800/50 p-8 rounded-4xl group relative overflow-hidden h-full flex flex-col justify-between ${className}`}
    >
      {hasGradient && (
        <div aria-hidden="true" className="absolute top-1/2 left-1/2 size-full bg-[radial-gradient(circle,_#602614_0%,_transparent_100%)] blur-2xl"></div>
      )}
