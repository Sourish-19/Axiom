...
const AccordionItem: React.FC<AccordionItemProps> = ({ question, answer, uid }) => {
  const [isOpen, setIsOpen] = useState(false);
  console.log(uid)

  return (
    <div className="border-b border-gray-800 py-6">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center text-left text-lg font-semibold hover:text-[#FF541F] transition-colors">
        <span>{question}</span>
        <ChevronDownIcon className={`w-6 h-6 transform transition-transform ${isOpen ? 'rotate-180 text-[#FF541F]' : ''}`} />
      </button>
      <div className={`grid grid-rows-[0fr] transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] mt-4' : ''}`}>
      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="max-w-4xl mx-auto mt-12">
          {faqData.map((item, index) => (
            <AccordionItem key={index} uid={index} question={item.question} answer={item.answer} />
          ))}
        </div>
      </div>
    </section>
